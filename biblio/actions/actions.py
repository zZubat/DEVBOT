...
class ActionBuscarPorTitulo(Action):
    ...
    def run(self, dispatcher: CollectingDispatcher, tracker: Tracker, domain: dict) -> List[Dict]:
        ...
        if data["numFound"] > 0:
            livros = data["docs"][:3]
            mensagens = [
                f"- {livro.get('title')} por {', '.join(livro.get('author_name', ['Autor desconhecido']))}"
                for livro in livros
            ]
...
class ActionBuscarPorAutor(Action):  
    ...
    def run(self, dispatcher: CollectingDispatcher, tracker: Tracker, domain: Dict) -> List[Dict]:  
        ...
        if livros["numFound"] > 0:    
            livros = livros["docs"][:3]  
...
class ActionBuscarPorAssunto(Action):  
    ...
    def run(self, dispatcher: CollectingDispatcher, tracker: Tracker, domain: Dict) -> List[Dict]:  
        ...
        if not assunto:  
            dispatcher.utter_message(text="Qual assunto você quer buscar?")  
            return []  
